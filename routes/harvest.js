      ...
      models.FoodprintHarvest.findAll(sql_search_condition)
        .then(rows => {
          ...
          res.render('harvestlogbook', {
            page_title: 'MercTest - Harvest Logbook',
            data: rows,
            user: req.user,
    ...
    try {
      ...
      tw_client
        ...
        .then(media => {
          ...
          const filenames = resolveFilenames(harvest_logid_uuid, `.${ext}`);
          const uploadParams = getUploadParams(
            BucketName,
            contentType,
            harvest_photoHash,
  ...
  function (req, res) {
    models.FoodprintHarvest.findAll({
      ...
      .then(rows => {
        ...
        const pdfFilename = `MercTest_Harvest_Admin_${username}_${moment(new Date()).format(
          'YYYY-MM-DD'
        )}`;
  ...
  function (req, res) {
    models.FoodprintHarvest.findAll({
      ...
      .then(rows => {
        ...
        const username = req.user.username;
        const useremail = req.user.email;
        const pdfFilename = `MercTest_Harvest_${username}_${moment(new Date()).format(
          'YYYY-MM-DD'
        )}`;
        const filenames = resolveFilenames(pdfFilename, pdffileextension);
    ...
    models.User.findOne({
      ...
      .then(user => {
        ...
        models.FoodprintHarvest.findAll({
          ...
          .then(rows => {
            const pdfFilename = `MercTest_Harvest_${phoneNumber}`;
            const pdffileextension = '.pdf';
...
router.post('/getimagehash', async function (req, res, next) {
  try {
    const image_hash = new Buffer('random shit');
