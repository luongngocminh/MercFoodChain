  ...
  function (req, res, next) {
    if (req.user.role === ROLES.Admin || req.user.role === ROLES.Superuser) {
      models.FoodprintProduce.findAll({
        ...
        .then(rows => {
          res.render('produce', {
            page_title: 'MercTest - Produce Page',
            data: rows,
            user: req.user,
            filter_data: '',
            page_name: 'produce',
      ...
      models.FoodprintProducePrice.findAll({
        ...
        .then(rows => {
          res.render('produce_price', {
            page_title: 'MercTest - Produce Price Page',
            data: rows,
            user: req.user,
            filter_data: '',
  ...
  function (req, res) {
    ...
    if (!result.isEmpty()) {
      ...
      res.render('pricepage', {
        page_title: 'MercTest - Price Page',
        data: '',
        page_name: 'pricepage',
      }); //should add error array here
    } else {
...
router.get(
  ...
  function (req, res, next) {
    models.FoodprintProducePrice.findAll({
      ...
      .then(rows => {
        let pdfFilename =
          'MercTest_ProducePrice_' + moment(new Date()).format('YYYY-MM-DD') + '.pdf';
...
router.get('/pricepage/pdf/whatsapp', function (req, res, next) {
  try {
    models.FoodprintProducePrice.findAll({
      ...
      .then(rows => {
        let pdfFilename = 'MercTest_ProducePrice_' + moment(new Date()).format('YYYY-MM-DD');
