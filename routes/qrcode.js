      ...
      models.FoodprintQRCode.findAll({
        ...
        .then(async rows => {
          ...
          res.render('dashboard_qrcode_static', {
            page_title: 'MercTest - QR Code Dashboard',
            data: rows,
            user: req.user,
            qrcodes: qrcodes,
            filter_data: '',
...
router.post(
  ...
  function (req, res) {
    ...
    if (!result.isEmpty()) {
      ...
      res.render('dashboard_qrcode_static', {
        page_title: 'MercTest - QR Code Configuration Dashboard',
        data: '',
        user: req.user,
        page_name: 'dashboard_qrcode_static',
  ...
  function (req, res) {
    ...
    if (!result.isEmpty()) {
      ...
      try {
        ...
        //res.json({success: false, errors: e});
        res.render('dashboard_qrcode_attributes', {
          page_title: 'MercTest - QR Code Attribute Dashboard',
          data: '',
          success: false,
          errors: e.array(),
          page_name: 'dashboard_qrcode_attributes',
        ...
        .then(rows =>
          models.FoodprintQRCode.findAll({
            ...
            res.render('qrcode_product', {
              page_title: 'MercTest - QR Code Product',
              product_data: rows,
              qrdata: req.params.qrid,
              user: req.user,
      ...
      models.FoodprintQrcount.findAll({
        ...
        .then(rows => {
          res.render('dashboard_qrcode_scans', {
            page_title: 'MercTest - QR Code Analytics Dashboard',
            data: rows,
            user: req.user,
            filter_data: '',
